...
class PacketCapture:
	...
	def __init__(self, config):
		...
		if self.pcap_enabled:
			...
			# Optional callback invoked whenever a PCAP file is rotated (closed)
			self.on_pcap_rotated: Optional[Callable[[str], None]] = None
	...
	def stop(self):
		...
		if self.pcap_writer:
			...
			self.pcap_writer = None
			# Fire rotation callback on final file close
			closed_path = str(self.current_pcap_file) if self.current_pcap_file else None
			if closed_path and self.on_pcap_rotated:
				try:
					threading.Thread(target=self.on_pcap_rotated, args=(closed_path,), daemon=True).start()
				except Exception as cb_e:
					logger.error(f"Error invoking on_pcap_rotated callback: {cb_e}")
	...
	def _rotate_pcap_file(self):
		...
		if self.pcap_writer:
			...
			previous_file = str(self.current_pcap_file)
			# Fire rotation callback in background
			if previous_file and self.on_pcap_rotated:
				try:
					threading.Thread(target=self.on_pcap_rotated, args=(previous_file,), daemon=True).start()
				except Exception as cb_e:
					logger.error(f"Error invoking on_pcap_rotated callback: {cb_e}")
